...
subtitle: "Predicting Stock Returns via Greedy Algorithm with
 Taiwanese News Data"
author: "研究生：程長磊(指導教授：林士貴博士、翁久幸博士)"
fontsize: 20pt
format: 
  revealjs: 
    theme: "black"
...
每篇新聞對應一個標籤 $y_i$，表示其情緒傾向（例如：正面、負面、漲、跌）：
...
- $y_i$：第 $i$ 篇新聞的情緒標籤（可為 0/1 或連續值）(二分類報酬或連續報酬)
...
用來決定最佳模型大小，避免過度擬合。懲罰項包含 \$ \log(p) \$，反映高維度空間的複雜性。

$$
\text{HDIC}(J) = -\ell(\hat{\beta}_J) + \#(J) \cdot w_n \cdot \left( \log\left(\frac{p}{n}\right) \right)^{1/2}
$$
